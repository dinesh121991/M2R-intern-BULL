...
int main( int argc, char*argv[] )
{
	...
	int nb_of_nodes=3000,nb_of_sockets=nb_of_nodes*sockets_per_node,
...
//                j= 2*i;
                ...
                printf("Entity=socket%d type=Socket socketcount=1 NumSumCores=0 SelectedSumCores=0 AllocatedSumCores=0  BitmapPosition=0 SelectedSocketCount=0 AllocatedSocketCount=0 AllocatedSumThreads=0 SelectedSumThreads=0 NumSumThreads=0 Enclosed=core[%d",i,j);
		printf("-%d]\n",(j += cores_per_socket - 1));
        }

printf("#Core level Layout configuration following\n");

	printf("Entity=core[0-%d] type=Core ThreadsPerCore=1 SelectedThreadsPerCore=0 AllocatedThreadsPerCore=0  BitmapPosition=0 SelectedCoreCount=0 AllocatedCoreCount=0 corecount=1",nb_of_cores-1);
