/*****************************************************************************\
 *  select_cons_res_layout.c - node selection plugin supporting consumable
 *  resources policies by using Layout concepts.
 *****************************************************************************\
 *
...
#include <string.h>
//#include <stdlib.h>  //If itoa is supported
...
uint16_t *cr_node_num_sockets __attribute__((weak_import));
uint32_t *cr_node_sockets_offset __attribute__((weak_import));
#else
...
uint16_t *cr_node_num_sockets;
uint32_t *cr_node_sockets_offset;
List config_list;	/* list of config_record entries */
List front_end_list;	/* list of slurm_conf_frontend_t entries */
#endif
 ...
 */
const char plugin_name[] = "Consumable Resources Layout (CRL) Node Selection plugin";
const char plugin_type[] = "select/cons_res_layout";
...
uint16_t *cr_node_num_sockets;
uint32_t *cr_node_sockets_offset;
int root_switch_index;
int leaf_switch_cnt;
int *leaf_switch_index;
...
bitstr_t * _nodemap_to_coremap(bitstr_t *node_bitmap)
{
	bitstr_t *core_bitmap;
	int node_count = bit_size(node_bitmap);
	int core_count = cr_get_coremap_offset (node_count);
	int i,j;
	char str[10]=L_CORE,str_i[18];

	
	core_bitmap = bit_alloc(core_count);
	bit_nclear(core_bitmap,0,core_count-1);

	info("Core Count:%d Node_count:%d",core_count,node_count);
	for( i=0 ; (i < node_count) && (bit_test(node_bitmap,i)) ; i++ )
	{
		int nodes_corresponding_core_offset = cr_get_coremap_offset ( i );
		for( j = cr_node_num_cores[i]-1 ; j >= 0 ; j-- )
		{
			nodes_corresponding_core_offset += j ;
			sprintf(str_i,"%s%d",str,nodes_corresponding_core_offset);
			int allocated_core_count = 0 ;
			if( layouts_entity_get_kv (L_CONS_RES_PARTITION,str_i ,L_ALLOCATED_CORE_COUNT,&allocated_core_count,L_T_UINT32) == SLURM_SUCCESS &&
			   allocated_core_count!=0 )
			{
				bit_set(core_bitmap,nodes_corresponding_core_offset);
			}
			else
				continue;
		}
...
void _dump_layout_correlated_global_info(struct job_record *job_ptr,  bitstr_t * bitmap )
{
	char str[45];
	bitstr_t *node_to_core_map;
	
	int bitmap_position,i,j;
	
	int free_sum_nodes_cluster;
	if( job_ptr )
	{
		bitstr_t *core_bitmap;
		if(bitmap)
		{	
			core_bitmap = _nodemap_to_coremap(bitmap);
			bit_fmt(str,sizeof(str),bitmap);
			for( i=0; i< bit_size(bitmap) && bit_test(bitmap,i) ;i++)
			{
				layouts_entity_get_kv(L_CONS_RES_PARTITION, node_record_table_ptr[i].name, L_BITMAP_POSITION, &bitmap_position,0);
				info("NodeRecordIndex:%d NodeRecordName:%s LayoutBitmapPosition:%d",i,node_record_table_ptr[i].name,bitmap_position);
				info("NodeRecordIndex:%d NodeRecordName:%s Sockets:%d Sockets-offset:%d",i,node_record_table_ptr[i].name,cr_node_num_sockets[i],cr_node_sockets_offset[i]);
			}
			node_to_core_map = _nodemap_to_coremap(bitmap);
			_dump_layout_correlated_core_info(node_to_core_map);
		}
		else
			sprintf(str ,"[No-Bitmap]");

		info("L-Node Map:%s of Job_Id:%d",str,job_ptr->job_id);
		
				
	}
	else
		info("Job submited details are lost or not  available [job_ptr=NULL]");
	
	
}

void _dump_layout_correlated_core_info (bitstr_t *core_bitmap)
{
	char str[175];
	int i;
	char *str1=L_CORE,str_i[20];
	int layout_core_map_position;
	int core_count = bit_size(core_bitmap);
	...
	if(core_bitmap)
		{			
			...
			{					
				sprintf(str_i,"%s%d",str1,i);
				layouts_entity_get_kv (L_CONS_RES_PARTITION,str_i,L_BITMAP_POSITION,&layout_core_map_position,L_T_UINT32); 
				info("Core-map-index:%d Core-name:%s, Layout-position:%d",i,str_i,layout_core_map_position);
			...
			sprintf(str,"[No-Bitmap]");
		info("L-Core Map:%s and NumberCores:%d ",str,core_count);
}
...
int _add_job_l(struct job_resources *job)
{
	/* core will be updated in the Layouts */	
	int core_first_set,core_last_set,node_first_set,
		node_last_set, socket_begin,socket_end,core_begin,core_end;
	int allocated_sum_cores_socket,num_sum_cores_socket,allocated_sum_cores_node,
		num_sum_cores_node,allocated_sum_sockets_node,num_sum_sockets_node;
	int allocated_core_count=1,allocated_socket_count=1,allocated_node_count=1;
	int i,j,k,slurm_status=SLURM_SUCCESS;
	char str_i[20];
	char core[145],node[41];
	int node_bitmap_size,core_bitmap_size,core_set_size,node_set_size;
	
	core_first_set= bit_ffs(job->core_bitmap);
	
	node_first_set = bit_ffs(job->node_bitmap);
	
	core_bitmap_size= bit_size(job->core_bitmap);
	node_bitmap_size= bit_size(job->node_bitmap);
	core_set_size = bit_set_count(job->core_bitmap);
	node_set_size = bit_set_count (job->node_bitmap);

	
	if (core_first_set >= 0)
		core_last_set = bit_fls(job->core_bitmap);
	else
		core_last_set = core_first_set - 1;
	
	if (node_first_set >= 0)
		node_last_set = bit_fls(job->node_bitmap);
	else
		node_last_set = node_first_set - 1;

	/*core_last_set = bit_size(job->core_bitmap);
	node_last_set = bit_size(job->node_bitmap);*/
	if(job->node_bitmap && job->core_bitmap)
	{
		bit_fmt(node,sizeof(node),job->node_bitmap);
		bit_fmt(core,sizeof(core),job->core_bitmap);
	}
	info("L-_add_job_l core_first_set:%d core_last_set:%d node_map:%s core_map:%s node_first_set:%d node_last_set:%d SizeOfNode:%d&Core:%d"
	     ,core_first_set,core_last_set,node,core,node_first_set,node_last_set,node_bitmap_size,core_bitmap_size);
	info("  Allocated Node-Size:%d Cor-size:%d",node_set_size,core_set_size);
	
	for(i=core_first_set; i<=core_last_set ; i++)
	{
		if(bit_test(job->core_bitmap,i))
		{
			...
			continue ;  // change like this 
	}
		
	for(i=node_first_set ; i<= node_last_set ; i++) 
	{
		if( bit_test(job->node_bitmap,i) )
		{
			socket_begin = cr_get_socketmap_offset (i);
			socket_end = cr_get_socketmap_offset (i+1);
			
			for(j=socket_begin; j<socket_end; j++)
			{
				sprintf(str_i,"%s%d",L_SOCKET,j);
				info("L-Job_res socket:%s is going to allocate",str_i);
				if(layouts_entity_pullget_kv(L_CONS_RES_PARTITION,str_i,L_ALLOCATED_SUM_CORES_SOCKET,
					                         &allocated_sum_cores_socket,L_T_UINT32)==SLURM_SUCCESS &&
				   layouts_entity_pullget_kv(L_CONS_RES_PARTITION,str_i,L_NUM_SUM_CORES_SOCKET,
					                         &num_sum_cores_socket,L_T_UINT32)==SLURM_SUCCESS)
				{
					if( allocated_sum_cores_socket > 0 &&
					   layouts_entity_setpush_kv (L_CONS_RES_PARTITION,str_i,L_ALLOCATED_SOCKET_COUNT,
						                          &allocated_socket_count,L_T_UINT32) == SLURM_SUCCESS
					   )
					{
						info("L-Job_res socket:%s is allocated:%d",str_i,allocated_socket_count);
					}
					else 
						continue;
				}
			...
			if(layouts_entity_pullget_kv(L_CONS_RES_PARTITION,node_record_table_ptr[i].name,L_ALLOCATED_SUM_CORES_NODE,
					                         &allocated_sum_cores_node,L_T_UINT32)==SLURM_SUCCESS &&
				layouts_entity_pullget_kv(L_CONS_RES_PARTITION,node_record_table_ptr[i].name,L_NUM_SUM_CORES_NODE,
					                         &num_sum_cores_node,L_T_UINT32)==SLURM_SUCCESS &&
				layouts_entity_pullget_kv(L_CONS_RES_PARTITION,node_record_table_ptr[i].name,L_ALLOCATED_SUM_SOCKETS_NODE,
					                         &allocated_sum_sockets_node,L_T_UINT32)==SLURM_SUCCESS &&
				layouts_entity_pullget_kv(L_CONS_RES_PARTITION,node_record_table_ptr[i].name,L_NUM_SUM_SOCKETS_NODE,
					                         &num_sum_sockets_node,L_T_UINT32)==SLURM_SUCCESS)
			{
				if( allocated_sum_cores_node == num_sum_cores_node &&
					   layouts_entity_setpush_kv (L_CONS_RES_PARTITION,node_record_table_ptr[i].name,L_ALLOCATED_NODE_COUNT,
						                          &allocated_node_count,L_T_UINT32) == SLURM_SUCCESS
					   )
					{
						info("L-Job_res node:%s is allocated:%d",node_record_table_ptr[i].name,allocated_node_count);
					}
	...
	if(layouts_entity_pullget_kv (L_CONS_RES_PARTITION,L_CLUSTER,L_ALLOCATED_SUM_CORES_CLUSTER,&allocated_sum_cores,L_T_UINT32)==SLURM_SUCCESS &&
	...
	{
		info("L-Job_res Cluster level allocated_sum_cores:%d allocated_sum_sockets:%d allocated_sum_nodes:%d",
		     allocated_sum_cores,allocated_sum_sockets,allocated_sum_nodes);
	}
	else 
		return SLURM_ERROR;
	return slurm_status;
...
static int _add_job_to_res(struct job_record *job_ptr, int action)
{
	...
	/*layout operation */
	if( job && _add_job_l(job) == SLURM_SUCCESS )
	{
		info("L-Job_res added allocated resources for the job to layout");
	}
	...
	if (action != 1) {
		...
		if (!p_ptr) {
			error("L-cons_res: could not find cr partition %s",
		...
		for (i = 0; i < p_ptr->num_rows; i++) {
			if (!_can_job_fit_in_row(job, &(p_ptr->row[i])))
				continue;
			debug3("L-cons_res: adding job %u to part %s row %u",
 ...
 * from Layout 
 */
int _rm_job_l(struct job_resources *job)
{
	/* core will be updated in the Layouts */	
	int core_first_set,core_last_set,node_first_set,
		node_last_set, socket_begin,socket_end,core_begin,core_end;
	int allocated_sum_cores_socket,num_sum_cores_socket,allocated_sum_cores_node,
		num_sum_cores_node,allocated_sum_sockets_node,num_sum_sockets_node;
	int allocated_core_count=0,allocated_socket_count=0,allocated_node_count=0;
	int i,j,k,slurm_status=SLURM_SUCCESS;
	char str_i[20];
	char core[145],node[41];
	int node_bitmap_size,core_bitmap_size,core_set_size,node_set_size;
	...
	info("L-_rm_job_l core_first_set:%d core_last_set:%d node_map:%s core_map:%s node_first_set:%d node_last_set:%d SizeOfNode:%d&Core:%d"
	     ,core_first_set,core_last_set,node,core,node_first_set,node_last_set,node_bitmap_size,core_bitmap_size);
	info("  Allocated Node-Size:%d Core-size:%d",node_set_size,core_set_size);
	...
	for(i=core_first_set; i<=core_last_set  ; i++)
	{
		if( bit_test(job->core_bitmap,i) )
		{
			...
			if(layouts_entity_setpush_kv (L_CONS_RES_PARTITION,str_i,
						                      ...
						                      L_T_UINT32) == SLURM_SUCCESS)
			{
				info("L-Job_res core:%s is removed:%d",str_i,allocated_core_count);
			}
			else 
				return SLURM_ERROR;
		}
		...
		if( bit_test(job->node_bitmap,i) )
		{
			...
			for(j=socket_begin; j<socket_end; j++)
			{
					                         ...
					                         &num_sum_cores_socket,L_T_UINT32)==SLURM_SUCCESS)
				{
					...
					{
						info("L-Job_res socket:%s is removed:%d",str_i,allocated_socket_count);
					}
					else 
						continue;
				}
					                         ...
					                         &num_sum_sockets_node,L_T_UINT32)==SLURM_SUCCESS)
			{
				if(allocated_sum_cores_node < num_sum_cores_node)
				{
					if(  layouts_entity_setpush_kv (L_CONS_RES_PARTITION,node_record_table_ptr[i].name,L_ALLOCATED_NODE_COUNT,
						                          &allocated_node_count,L_T_UINT32) == SLURM_SUCCESS)
						info("L-Job_res node:%s is removed:%d,because having free_sum_cores",node_record_table_ptr[i].name,allocated_node_count);
				}				
			}
			else 
				return SLURM_ERROR;
			    ...
			    struct job_record *job_ptr, int action)
{
	...
	if( job && _rm_job_l(job) == SLURM_SUCCESS )
	{
		info("L-Job_res removed allocated resources for the Job from layout");
	}
		      ...
		      uint32_t req_nodes, uint16_t job_node_req)
{
	...
	if(LAYOUT)
		rc = cr_job_test_l(job_ptr, bitmap, min_nodes, max_nodes, req_nodes,
		    ...
		    bitstr_t *exc_core_bitmap)
{
	...
	uint16_t tmp_cr_type = cr_type;
	int i, node_count,free_sum_cores_node;
	char str_i[20],str[64];
...
/* Layout is used to remove less number of cores containing nodes from the selection list */
/*	node_count = bit_size(bitmap);
	bit_fmt(str,sizeof(str),bitmap);
	info("L-_run_now-Before node availability check node_map:%s",str);
	for( i=0 ; i < node_count && bit_test(bitmap,i) ; i++ )
	{
		sprintf(str_i,"%s%d",L_NODE,i);
		if(layouts_entity_get_kv (L_CONS_RES_PARTITION,str_i,
		   L_FREE_SUM_CORES_NODE,&free_sum_cores_node,L_T_UINT32) == SLURM_SUCCESS &&
		   job_ptr->details->min_cpus <= free_sum_cores_node)
		{
			continue;
		}
		else
	...
	bit_fmt(str,sizeof(str),bitmap);
	info("L-_run_now-After node availability check node_map:%s",str);*/
	if(LAYOUT)
		rc = cr_job_test_l(job_ptr, bitmap, min_nodes, max_nodes, req_nodes,
	...
	if ((rc != SLURM_SUCCESS) && preemptee_candidates && preempt_by_qos &&
		...
		if(LAYOUT)
			rc = cr_job_test_l(job_ptr, bitmap, min_nodes, max_nodes,
		...
		while ((tmp_job_ptr = (struct job_record *)
			...
			bit_or(bitmap, orig_map);
			if(LAYOUT)
				rc = cr_job_test_l(job_ptr, bitmap, min_nodes,
					 max_nodes, req_nodes,
...
// Nothing to init for LYOUT_CONS_RES , because layout_init() perform everything 
	return SLURM_SUCCESS;
}
...
extern int fini(void)
{
	...
	xfree(leaf_switch_index);
	if (cr_type)
		verbose("%s shutting down ...", plugin_name);
//Nothing to finish for LYOUT_CONS_RES, Layout will finish
	return SLURM_SUCCESS;
}
...
{
	/* save Layout final information in the configuration format
	 */
	/*if ( layouts_state_save_layout("cons_res_partition") == SLURM_SUCCESS )
	{
		info("D-select_p_state_save dir: %s",dir_name);
		return SLURM_SUCCESS;
	}
	else
		return SLURM_ERROR;*/
...
{
	/* nothing to restore/save without Layout approach */
	/*if(layouts_load_config(1) == SLURM_SUCCESS)
		return SLURM_SUCCESS;
	else
		return SLURM_ERROR;*/
...
extern int select_p_node_init(struct node_record *node_ptr, int node_cnt)
{
	...
	int i,j, tot_core, num_sum_cores, num_sockets, core_start_offset,
		number_of_cores_socket, number_of_cores_node,nodes_corresponding_core_offset,
		nodes_corresponding_socket_offset,cps,cores_per_socket;
	int socket_begin,socket_end;
	char str_i[20];
	uint16_t sockets;

	info("cons_res: select_p_node_init");
	cr_node_num_sockets = xmalloc(node_cnt * sizeof(uint16_t));
	cr_node_sockets_offset = xmalloc((node_cnt+1) * sizeof(uint32_t));
	...
	info("Initialize Global core data");
	...
	/* switch_record_table initialization */
	root_switch_index = -2;
	leaf_switch_cnt = -2;
	
	for( i=0 ; i< switch_record_cnt ; i++)
	{
		if(switch_record_table[i].level == switch_levels)
		{
			root_switch_index = i;
			info("L- root_switch_index:%d",root_switch_index);
		}
		else if(switch_record_table[i].level == 0)
		{
			leaf_switch_cnt++;
		}
		else
	...
	info("L- leaf_switch_cnt:%d",leaf_switch_cnt);
	
	leaf_switch_index = xmalloc(leaf_switch_cnt*sizeof(int));

	for( i=0,j=0 ; i < switch_record_cnt && j <= leaf_switch_cnt ; i++)
	{
		if(switch_record_table[i].level == 0)
		{
			leaf_switch_index[j] = i;
			info("L- leaf_switch_index[j:%d]:%d",j,leaf_switch_index[j]);
			j++;
		}	
	}

	if( leaf_switch_cnt==-2 && root_switch_index==-2 )
	{
		/* Cluster level Node Information pulled */
		if(layouts_entity_pullget_kv (L_CONS_RES_PARTITION,L_CLUSTER,L_NUM_SUM_NODES_CLUSTER,&num_sum_nodes,L_T_UINT32) == SLURM_SUCCESS &&
		...
		{
			info("Layout -num_sum_nodes:%d node_record_count:%d num_sum_cores:%d num_sum_sockets:%d"
				 ,num_sum_nodes,node_record_count,num_sum_cores,num_sum_sockets);
			if(num_sum_nodes == node_record_count)
			{
						info("Layout is upto date with the Node records details- Illegal activities of Modification,Insert&Delete are not acceptable right now");
			}
			else
			{
				info("Layout is not synchronized with the Node records details, So please keep that to proceed");
				return SLURM_ERROR;
			}
			if(	/*(layouts_entity_pullget_kv(L_CONS_RES_PARTITION,L_CLUSTER,L_FREE_SUM_NODES_CLUSTER,&free_sum_nodes,L_T_UINT32) == SLURM_SUCCESS) &&*/
				(layouts_entity_pullget_kv(L_CONS_RES_PARTITION,L_CLUSTER,L_ALLOCATED_SUM_NODES_CLUSTER,&allocated_sum_nodes,L_T_UINT32) == SLURM_SUCCESS) 
			   )
				info("L_ALLOCATED_SUM_NODES_CLUSTER:%d ",allocated_sum_nodes);
			else
				info("Layouts Allocated or Free part in the cluster Does not work");
		}
	}
	else
	{
		/* Cluster level Node Information pulled */
		char num_sum_nodes_in_level[20];
		sprintf(num_sum_nodes_in_level,"%s%d",L_NUM_SUM_NODES_IN_LEVEL,switch_record_table[root_switch_index].level);

		char num_sum_cores_in_level[20];
		sprintf(num_sum_cores_in_level,"%s%d",L_NUM_SUM_CORES_IN_LEVEL,switch_record_table[root_switch_index].level);

		char num_sum_sockets_in_level[20];
		sprintf(num_sum_sockets_in_level,"%s%d",L_NUM_SUM_SOCKETS_IN_LEVEL,switch_record_table[root_switch_index].level);
		
		if(layouts_entity_pullget_kv (L_CONS_RES_PARTITION,switch_record_table[root_switch_index].name,num_sum_nodes_in_level,&num_sum_nodes,L_T_UINT32) == SLURM_SUCCESS &&
		   layouts_entity_pullget_kv (L_CONS_RES_PARTITION,switch_record_table[root_switch_index].name,num_sum_cores_in_level,&num_sum_cores,L_T_UINT32) == SLURM_SUCCESS &&
		   layouts_entity_pullget_kv (L_CONS_RES_PARTITION,switch_record_table[root_switch_index].name,num_sum_sockets_in_level,&num_sum_sockets,L_T_UINT32) == SLURM_SUCCESS
		   )
...
/* Bitmap calculation was started */
	for (i = 0; i < select_node_cnt; i++)
	{
		info("Start Layout bitmap and coremap position initialization ....");
		select_node_record[i].node_ptr = &node_ptr[i];

//Node map correlation with Layout
		if(node_ptr[i].name != NULL && node_ptr[i].name !="")
		{
			info("BitmapPosition:%d will be changed for the node:%s and it's node_hostname:%s",i,node_ptr[i].name,node_ptr[i].node_hostname);
			if ( layouts_entity_set_kv (L_CONS_RES_PARTITION,node_ptr[i].name,L_BITMAP_POSITION,&i,L_T_UINT32) == SLURM_SUCCESS )
			{
				if ( layouts_entity_get_kv (L_CONS_RES_PARTITION,node_ptr[i].name,L_BITMAP_POSITION,&j,L_T_UINT32) == SLURM_SUCCESS )
					info("BitmapPosition:%d was changed in the node:%s and it's node_hostname:%s",j,node_ptr[i].name,node_ptr[i].node_hostname);
			}
			else
				info("Bitmap is not properly updated in the Layout entities,Sorry for the inconvenience");
		}
// Core,Socket Bitmap position Correlation with Layout
		
		core_start_offset = cr_get_coremap_offset (i);
		for( j=0 ; j < cr_node_num_cores[i] ; j++ )
		{ 
			nodes_corresponding_core_offset = core_start_offset + j;
				
			sprintf(str_i,"%s%i",L_CORE,nodes_corresponding_core_offset);
			if (layouts_entity_set_kv (L_CONS_RES_PARTITION,str_i,L_BITMAP_POSITION,&nodes_corresponding_core_offset,L_T_UINT32) == SLURM_SUCCESS
				)
				info("Layouts Core Bitmap Position for Node-%d:%s and Core-%d:%s was updated correctly",i,node_ptr[i].name,
					     nodes_corresponding_core_offset,str_i);				
			}
		socket_begin = cr_get_socketmap_offset (i);
		socket_end = cr_get_socketmap_offset (i+1);
		for(j=0 ; j< socket_end ; j++)
		{
			sprintf(str_i,"%s%i",L_SOCKET,j);
			if (layouts_entity_set_kv (L_CONS_RES_PARTITION,str_i,L_BITMAP_POSITION,&j,L_T_UINT32) == SLURM_SUCCESS 
				)
					info("Layouts Socket Bitmap Position for Node-%d:%s and Socket:%s was updated correctly",i,node_ptr[i].name,
					     str_i);
		}
		...
		cr_node_num_sockets[i] = sockets;
		if (i > 0) {
			cr_node_sockets_offset[i] = cr_node_sockets_offset[i-1] +
				cr_node_num_sockets[i-1] ;
		} 
		else
		{
			cr_node_sockets_offset[0] = 0;
		}
...
uint32_t cr_get_socketmap_offset(uint32_t node_index)
{
	xassert(cr_node_sockets_offset);
	return cr_node_sockets_offset[node_index];
}
			     ...
			     bitstr_t *exc_core_bitmap)
{
	...
	_dump_layout_correlated_global_info(job_ptr, bitmap);

	/* Layout Initial value to check availability */
	...
	if (select_debug_flags & DEBUG_FLAG_SELECT_TYPE) {
		...
		info("cons_res: select_p_job_test: min_n %u max_n %u req_n %u "
		     "avail_n %u min_cpus:%d num_tasks:%d",min_nodes, max_nodes, req_nodes,
		     bit_set_count(bitmap),job_ptr->details->min_cpus,job_ptr->details->num_tasks);
		_dump_state(select_part_record);
	}
	if (mode == SELECT_MODE_WILL_RUN && job_allowed ) {
		rc = _will_run_test(job_ptr, bitmap, min_nodes, max_nodes,
				    req_nodes, job_node_req,
				    preemptee_candidates, preemptee_job_list,
				    exc_core_bitmap);
	} else if (mode == SELECT_MODE_TEST_ONLY && job_allowed ) {
		rc = _test_only(job_ptr, bitmap, min_nodes, max_nodes,
				req_nodes, job_node_req);
	} else if (mode == SELECT_MODE_RUN_NOW && job_allowed ) {
	...
	{
		info("Hi,Jim there is not enough resources in the cluster");
		return SLURM_ERROR;
	}
	else
	...
	if (select_debug_flags & DEBUG_FLAG_CPU_BIND) {
		if (job_ptr->job_resrcs)
			...
			info("No job_resources info for Job-%u SLURM_SUCCESS(rc)=%d",
			     job_ptr->job_id, rc);
		}
