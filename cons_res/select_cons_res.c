...
static void _dump_nodes(void)
{
	...
	for (i=0; i<select_node_cnt; i++) {
		...
		info("D-_dump_nodes-node:%s cpus:%u c:%u s:%u t:%u mem:%u a_mem:%u state:%d",
		     node_ptr->name,
...
{
	uint16_t i;
	info("D-_dump_part- part:%s rows:%u pri:%u num_rows:%d ", p_ptr->part_ptr->name, p_ptr->num_rows,
	     p_ptr->part_ptr->priority, p_ptr->num_rows);
	if (!p_ptr->row)
		return;
	...
	for (i = 0; i < p_ptr->num_rows; i++) {
		...
		info("D-_dump_part-  row%u: num_jobs %u: row_bitmap: %s Job_list_size:%d", i,
		     p_ptr->row[i].num_jobs, str,p_ptr->row[i].job_list_size);
	}
}
...
static struct part_res_record *_dup_part_data(struct part_res_record *orig_ptr)
{
	...
	info("D-_dup_part_data in ");
	if (orig_ptr == NULL)
		return NULL;
	info("D-_dup_part_data");
	_dump_state (orig_ptr);
...
static int _add_job_to_res(struct job_record *job_ptr, int action)
{
	...
	info("D-_add_job_to_res-in");
	if (!job || !job->core_bitmap) {
		error("D-job %u has no job_resrcs info", job_ptr->job_id);
		return SLURM_ERROR;
	}

	debug3("D-cons_res: _add_job_to_res: job %u act %d ", job_ptr->job_id,
	       action);
...
static job_resources_t *_create_job_resources(int node_cnt)
{
	...
	info("D-_create_job_resources- in");
				 ...
				 struct node_record *node_ptr)
{
	...
	info("D-_rm_job_from_one_node - in ");
	if (!job || !job->core_bitmap) {
...
static uint16_t _get_job_node_req(struct job_record *job_ptr)
{
	...
	info("D-_get_job_node_req- job_ptr->part_ptr->max_share:%d ",job_ptr->part_ptr->max_share);
...
static int _sort_usable_nodes_dec(void *j1, void *j2)
{
	...
	info("D-_sort_usable_nodes_dec- in ");
		    ...
		    bitstr_t *exc_core_bitmap)
{
	...
	info("D-_run_now for job id:%u, bitmap:%s,min_nodes:%u, max_nodes:%u, req_nodes:%u, job_node_req:%u"
	     ,job_ptr->job_id, str, min_nodes , max_nodes,req_nodes,job_node_req);
	/*bit_fmt(str, sizeof(str), exc_core_bitmap);
	info("D-_run_now for exc_core_bitmap:%s", str);*/

	save_bitmap = bit_copy(bitmap);
top:	orig_map = bit_copy(save_bitmap);
	
	bit_fmt(str, sizeof(str), bitmap);
	info("D-_run_now for bitmap:%s", str);
	info("D-_run_now-before-cr_job_test : tmp_cr_type:%u",tmp_cr_type);
	if (job_ptr->part_ptr->cr_type) {
		if ((cr_type & CR_SOCKET) || (cr_type & CR_CORE)) {
			tmp_cr_type &= ~(CR_SOCKET|CR_CORE);
			tmp_cr_type |= job_ptr->part_ptr->cr_type;
		} else {
			info("D-_run_now-cons_res: Can't use Partition SelectType unless using CR_Socket or CR_Core");
		}
	}

	info("D-_run_now-before-cr_job_test : job_ptr->part_ptr->cr_type:%d cr_type:%u tmp_cr_type:%u select_node_cnt:%d",
	       job_ptr->part_ptr->cr_type,cr_type,tmp_cr_type,select_node_cnt);
	...
	info("D-_run_now-after-cr_job_test bitmap size:%d",bit_size(bitmap));
	//_dump_state (select_part_record);
	bit_fmt(str, sizeof(str), bitmap);
	info("D-_run_now-after-cr_job_test for bitmap:%s", str);
	info("D-_run_now for job id:%u ,min_nodes:%u, max_nodes:%u, req_nodes:%u, job_node_req:%u, tmp_cr_type:%d, select_node_cnt:%d,"
	     , job_ptr->job_id, str,min_nodes,max_nodes,req_nodes,job_node_req,tmp_cr_type, select_node_cnt);
	//_dump_state (select_part_record);
/*	bit_fmt(str, sizeof(str), exc_core_bitmap);
	info("D-_run_now for exc_core_bitmap:%s", str);*/
	...
	if ((rc != SLURM_SUCCESS) && preemptee_candidates && preempt_by_qos) {
		...
		info("D-_run_now on (rc != SLURM_SUCCESS) && preemptee_candidates && preempt_by_qos) - list_count(preemptee_candidates) :%  d"
		...
		if ((tmp_job_ptr = (struct job_record *)
			...
			info("D-_run_now on (rc != SLURM_SUCCESS) && preemptee_candidates && preempt_by_qos) - tmp_job_ptr:%d,preempt_mode:%d"
		     ,tmp_job_ptr->job_id,mode);
		}
	...
	if ((rc != SLURM_SUCCESS) && preemptee_candidates && preempt_by_qos &&
		...
		info("D-_run_now on -(rc != SLURM_SUCCESS) && preemptee_candidates && preempt_by_qos && (mode == PREEMPT_MODE_SUSPEND)");
		bit_fmt(str, sizeof(str), bitmap);
		info("D-_run_now-PREEMPT_MODE_SUSPEND for bitmap:%s", str);
		bit_or(bitmap, orig_map);
		bit_fmt(str, sizeof(str), bitmap);
	    info("D-_run_now-PREEMPT_MODE_SUSPEND for bitmap |= orig_map:%s", str);
		bit_fmt(str, sizeof(str), orig_map);
		info("D-_run_now-PREEMPT_MODE_SUSPEND for orig_map:%s", str);
		...
		bit_fmt(str, sizeof(str), exc_core_bitmap);
		info("D-_run_now-PREEMPT_MODE_SUSPEND after cr_job_test for exc_core_bitmap:%s", str);

		bit_fmt(str, sizeof(str), bitmap);
		info("D-_run_now-PREEMPT_MODE_SUSPEND after cr_job_test for bitmap:%s", str);
		...
		info("D-_run_now- Preemptable jobs area - preemptee_cand_cnt : %d",preemptee_cand_cnt);
		...
		while ((tmp_job_ptr = (struct job_record *)
			...
			bit_fmt(str,sizeof(str),bitmap);
			info("D-_run_now- preempt bitmap :%s",str);
			...
			info("D-_run_now- preempt bitmap :%s",str);
			bit_fmt(str,sizeof(str),orig_map);
			info("D-_run_now- preempt orig_map :%s",str);
			...
			info("D-_run_now- preempt bitmap :%s",str);
			bit_fmt(str,sizeof(str),exc_core_bitmap);
			info("D-_run_now- preempt exc_core_bitmap :%s tmp_job_ptr->details->usable_nodes:%d preempt_strict_order:%s"
			     ,str,tmp_job_ptr->details->usable_nodes,preempt_strict_order);
	...
	if (rc != SLURM_SUCCESS) {
		...
		while ((tmp_job_ptr = list_next(job_iterator))) {
			...
			if (ovrlap == 0)	/* job has no usable nodes */
				continue;	/* skip it */
			info("cons_res: _will_run_test, job %u: overlap=%d",
...
extern int init(void)
{
	...
	info("D-init - cr_type":%d",cr_type);
	if (cr_type)
		verbose("%s loaded with argument %u", plugin_name, cr_type);
		info("D-select_cons_res-init : %s loaded with argument %u", plugin_name, cr_type);	
	select_debug_flags = slurm_get_debug_flags();
	info("D-select_cons_res-init : Debug flags %u", select_debug_flags);
	return SLURM_SUCCESS;
}
...
extern int fini(void)
{
	...
	if (cr_type)
		...
		info("D-select_cons_res-fini : %s shutting down ...", plugin_name);
	info("D-select_cons_res-fini : finished  ");
...
{
	/* nothing to save */
	info("D-select_cons_res-select_p_state_save :Directory Name - %s", dir_name);
	return SLURM_SUCCESS;
}
...
{
	/* nothing to restore */
	info("D-select_cons_res-select_p_state_restore :Directory Name - %s", dir_name);
	return SLURM_SUCCESS;
}
...
extern int select_p_job_init(List job_list)
{
	...
	info("D-select_cons_res-select_p_job_init :job_list pointer - %u",job_list);
	info("D-select_cons_res-select_p_job_init :List - %u", list_count(job_list));
	return SLURM_SUCCESS;
}
...
{
	info("D-select_cons_res-select_p_node_ranking : node_record-Host_Name -%s,node_recrd-State-%u , node_cnt - %d", node_ptr->node_hostname,node_ptr->node_state,node_cnt);
	return false;
}
...
extern int select_p_node_init(struct node_record *node_ptr, int node_cnt)
{
	...
	info("D-cons_res: select_p_node_init");
	info("D-select_cons_res-select_p_node_init : start node_record-HostName-%s, node_record-State,node_cnt:%u", node_ptr->node_hostname, node_ptr->node_state,node_cnt);
	...
	info("D-select_cons_res-select_p_node_init : End");
	return SLURM_SUCCESS;
}

extern int select_p_block_init(List part_list)
{
	info("D-select_cons_res-select_p_bloc_init :");
	return SLURM_SUCCESS;
}
			     ...
			     bitstr_t *exc_core_bitmap)
{
	...
	bit_fmt(str, sizeof(str), bitmap);
	info("D-select_p_job_test for bitmap:%s", str);
	info("D-select_p_job_test for job id:%u, bitmap:%s,min_nodes:%u, max_nodes:%u, req_nodes:%u, mode:%u, preemptee_candidates-count:(SF),preemptee_candidates-head_value:%u	 preemptee_job_list-count:(SF), preemptee_job_list-head_value:%u", job_ptr->job_id, str, min_nodes , max_nodes,req_nodes,mode,preemptee_candidates,*preemptee_job_list);

	/*bit_fmt(str, sizeof(str), exc_core_bitmap);
	info("D-select_p_job_test for exc_core_bitmap:%s", str);*/
	if (!debug_check) {
		debug_check = true;
		if (slurm_get_debug_flags() & DEBUG_FLAG_SELECT_TYPE)
			debug_cpu_bind = true;
	}
	info("D-select_p_job_test for debug_check:%d, debug_cpu_bind:%s, slurm_get_debug_flags():%u DEBUG_FLAG_SELECT_TYPE:%d,select_debug_flags:%u ", debug_check,debug_cpu_bind,slurm_get_debug_flags(),DEBUG_FLAG_SELECT_TYPE,select_debug_flags);
	if (!job_ptr->details)
		return EINVAL;
	...
	if ((job_ptr->details->core_spec != (uint16_t) NO_VAL) &&
		...
		info("D-select_p_job_test-Setting Exclusive mode for job Id: %u with CoreSpecResources=%u,WholeNode=%u",
		      job_ptr->job_id, job_ptr->details->core_spec, job_ptr->details->whole_node);
	...
	if (select_debug_flags & DEBUG_FLAG_SELECT_TYPE) {
		info("D-select_p_job_test: select_p_job_test: job %u job_node_req %u mode %d",
		     job_ptr->job_id, job_node_req, mode);
		info("D-select_p_job_test: select_p_job_test: min_n %u max_n %u req_n %u "
		     "avail_n %u",
	...
	if (mode == SELECT_MODE_WILL_RUN) {
		bit_fmt(str, sizeof(str), bitmap);
		info("D-select_p_job_test-SELECT_MODE_WILL_RUN-before: for bitmap:%s", str);
/*	info("D-select_p_job_test-SELECT_MODE_WILL_RUN-before: job id:%u, bitmap:%s,min_nodes:%u, max_nodes:%u, req_nodes:%u, mode:%u, preemptee_candidates-count:%u,	 preemptee_job_list-count:%u", job_ptr->job_id, str, min_nodes , max_nodes,req_nodes,mode,list_count(preemptee_candidates),list_count(*preemptee_job_list));*/
		/*bit_fmt(str, sizeof(str), exc_core_bitmap);
	info("D-select_p_job_test-SELECT_MODE_WILL_RUN-before: for exc_core_bitmap:%s", str);*/
		...
		bit_fmt(str, sizeof(str), bitmap);
		info("D-select_p_job_test-SELECT_MODE_WILL_RUN-after: for bitmap:%s", str);
	info("D-select_p_job_test-SELECT_MODE_WILL_RUN-after: job id:%u, bitmap:%s,min_nodes:%u, max_nodes:%u, req_nodes:%u, mode:%u, preemptee_candidates-count:%u,	 preemptee_job_list-count:%u", job_ptr->job_id, str, min_nodes , max_nodes,req_nodes,mode,list_count(preemptee_candidates),list_count(*preemptee_job_list));
		bit_fmt(str, sizeof(str), exc_core_bitmap);
	info("D-select_p_job_test-SELECT_MODE_WILL_RUN-after: for exc_core_bitmap:%s", str);
	} else if (mode == SELECT_MODE_TEST_ONLY) {
			bit_fmt(str, sizeof(str), bitmap);
		info("D-select_p_job_test-SELECT_MODE_TEST_ONLY-before: for bitmap:%s", str);
	/*info("D-select_p_job_test-SELECT_MODE_WILL_RUN-before: job id:%u, bitmap:%s,min_nodes:%u, max_nodes:%u, req_nodes:%u, mode:%u, preemptee_candidates-count:%u,	 preemptee_job_list-count:%u", job_ptr->job_id, str, min_nodes , max_nodes,req_nodes,mode,list_count(preemptee_candidates),list_count(*preemptee_job_list));*/
	/*	bit_fmt(str, sizeof(str), exc_core_bitmap);
	info("D-select_p_job_test-SELECT_MODE_WILL_RUN-before: for exc_core_bitmap:%s", str);*/ //exc_core_bitmap is not allocated

		rc = _test_only(job_ptr, bitmap, min_nodes, max_nodes,
				req_nodes, job_node_req);
		
		bit_fmt(str, sizeof(str), bitmap);
		info("D-select_p_job_test-SELECT_MODE_TEST_ONLY-after: for bitmap:%s", str);
	info("D-select_p_job_test-SELECT_MODE_TEST_ONLY-after: job id:%u, bitmap:%s,min_nodes:%u, max_nodes:%u, req_nodes:%u, mode:%u, preemptee_candidates-count:%u,	 preemptee_job_list-count:%u", job_ptr->job_id, str, min_nodes , max_nodes,req_nodes,mode,list_count(preemptee_candidates),list_count(*preemptee_job_list));
		bit_fmt(str, sizeof(str), exc_core_bitmap);
	info("D-select_p_job_test-SELECT_MODE_TEST_ONLY-after: for exc_core_bitmap:%s", str);
		
	} else if (mode == SELECT_MODE_RUN_NOW) {

		bit_fmt(str, sizeof(str), bitmap);
		info("D-select_p_job_test-SELECT_MODE_RUN_NOW-before: for bitmap:%s", str);
	/*info("D-select_p_job_test-SELECT_MODE_WILL_RUN-before: job id:%u, bitmap:%s,min_nodes:%u, max_nodes:%u, req_nodes:%u, mode:%u, preemptee_candidates-count:%u,	 preemptee_job_list-count:%u", job_ptr->job_id, str, min_nodes , max_nodes,req_nodes,mode,list_count(preemptee_candidates),list_count(*preemptee_job_list));*/
		/*bit_fmt(str, sizeof(str), exc_core_bitmap);
	info("D-select_p_job_test-SELECT_MODE_WILL_RUN-before: for exc_core_bitmap:%s", str);*/
	...
	rc = _run_now(job_ptr, bitmap, min_nodes, max_nodes,
		...
		bit_fmt(str, sizeof(str), bitmap);
		info("D-select_p_job_test-SELECT_MODE_RUN_NOW-after: for bitmap:%s", str);
	/*info("D-select_p_job_test-SELECT_MODE_WILL_RUN-after: job id:%u, bitmap:%s,min_nodes:%u, max_nodes:%u, req_nodes:%u, mode:%u, preemptee_candidates-count:%u,	 preemptee_job_list-count:%u", job_ptr->job_id, str, min_nodes , max_nodes,req_nodes,mode,list_count(preemptee_candidates),list_count(*preemptee_job_list));*/
		/*bit_fmt(str, sizeof(str), exc_core_bitmap);
	info("D-select_p_job_test-SELECT_MODE_RUN_NOW-after: for exc_core_bitmap:%s", str);*/
		
	} else
		fatal("D-select_p_job_test: Mode %d is invalid", mode);

	if (select_debug_flags & DEBUG_FLAG_CPU_BIND) {
		if (job_ptr->job_resrcs)
			log_job_resources(job_ptr->job_id, job_ptr->job_resrcs);
		else {
			info("D-select_p_job_test:no job_resources info for job %u rc=%d",
			     job_ptr->job_id, rc);
		}
	} else if (debug_cpu_bind && job_ptr->job_resrcs) {
		log_job_resources(job_ptr->job_id, job_ptr->job_resrcs);
	}
	info("D-select_p_job_test : return rc=%d",rc);
	return rc;
}

extern int select_p_job_begin(struct job_record *job_ptr)
{
	info("D-select_p_job_begin : current_cpu_cnt:%d,cr_enabled:%d,alloc_node:%s", job_ptr->cpu_cnt,job_ptr->cr_enabled,
	       job_ptr->alloc_node);
	return SLURM_SUCCESS;
}
...
extern int select_p_job_ready(struct job_record *job_ptr)
{
	...
	info("D-select_p_job_ready : current_cpu_cnt:%d,cr_enabled:%d,alloc_node:%s", job_ptr->cpu_cnt,job_ptr->cr_enabled,
	       job_ptr->alloc_node);
				...
				struct node_record *node_ptr)
{
	...
	info("D-select_p_job_resized : current_cpu_cnt:%d,cr_enabled:%d,alloc_node:%s", job_ptr->cpu_cnt,job_ptr->cr_enabled,
	       job_ptr->alloc_node);
...
{
	info("D-select_p_job_expand_allow - return true only");
	return true;
}
			       ...
			       struct job_record *to_job_ptr)
{
	...
	info("D-select_p_job_expand-From : current_cpu_cnt:%d,cr_enabled:%d,alloc_node:%s"
		"To : current_cpu_cnt:%d,cr_enabled:%d,alloc_node:%s", 
		from_job_ptr->cpu_cnt,from_job_ptr->cr_enabled,from_job_ptr->alloc_node , to_job_ptr->cpu_cnt,to_job_ptr->cr_enabled,
	       to_job_ptr->alloc_node);
...
extern int select_p_job_signal(struct job_record *job_ptr, int signal)
{
	...
	info("D-select_p_job_signal : current_cpu_cnt:%d,cr_enabled:%d,alloc_node:%s", job_ptr->cpu_cnt,job_ptr->cr_enabled,
	       job_ptr->alloc_node);
	return SLURM_SUCCESS;
}
...
extern int select_p_job_fini(struct job_record *job_ptr)
{
	xassert(job_ptr);
	xassert(job_ptr->magic == JOB_MAGIC);
	info("D-select_p_job_fini -before : current_cpu_cnt:%d,cr_enabled:%d,alloc_node:%s", job_ptr->cpu_cnt,job_ptr->cr_enabled,
	       job_ptr->alloc_node);
	_rm_job_from_res(select_part_record, select_node_usage, job_ptr, 0);

	info("D-select_p_job_resized -after");
	_dump_state (select_part_record);
...
extern int select_p_select_nodeinfo_set_all(void)
{
	...
	if (last_set_all && (last_node_update < last_set_all)) {
		info("Node select info for set all hasn't "
		       "changed since %ld",
		       (long)last_set_all);
			   ...
			   uint32_t *core_cnt, bitstr_t **core_bitmap)
{
	...
	if ((node_cnt) && (core_cnt)) {
		...
		info("Reserving %u cores across %d nodes",
	...
	if ((!node_cnt) && (core_cnt)) {
		...
		info("Reserving cores from nodes: %s", str);
		for (i = 0; (i < num_nodes) && core_cnt[i]; i++)
			total_core_cnt += core_cnt[i];
	}

	info("Reservations requires %d cores (%u each on %d nodes, plus %u)",
	...
	if (core_cnt) { /* Reservation is using partial nodes */
		...
		info("Reservation is using partial nodes");
		...
		info("tmpcore contains just current free cores: %s", str);
		...
		while (total_core_cnt) {
			...
			if (local_cores < cores_per_node) {
				info("Skip node %d (local: %d, needed: %d)",
			...
			if (cores_in_node < cores_per_node) {
				info("Skip node %d (avail: %d, needed: %d)",
					inx, cores_in_node, cores_per_node);
				continue;
			}

			info("Using node %d (avail: %d, needed: %d)",
				inx, cores_in_node, cores_per_node);
			...
			if (cores_in_node) {
				...
				info("Reservation using %d cores in node %d",
				       cores_in_node, inx);
				bit_set(sp_avail_bitmap, inx);
			} else {
				info("Reservation NOT using node %d", inx);
			}
			node_list_inx++;
		}
				     ...
				     bitstr_t **core_bitmap)
{
	...
	for (i = 0; i < switch_record_cnt; i++) {
		...
		info("Switch %d can use cores: %s", i, str);

		switches_cpu_cnt[i] = bit_set_count(switches_core_bitmap[i]);

		info("switch %d available cores in %d nodes: %d",
	...
	while (rem_nodes > 0) {
		...
		for (i=first; ((i<=last) && (first>=0)); i++) {
			...
			if (*core_bitmap) {
				...
				info("Testing node %d, core offset %d",
				       i, coff);
				...
				info("Using node %d with %d cores available",
...
fini:	for (i=0; i<switch_record_cnt; i++) {
	...
	if (avail_nodes_bitmap && core_cnt) {
		...
		while (core_cnt[0]) {
			...
			info("Using node inx %d cores_per_node %d "
			...
			info("Node %d has %d available cores", inx,
			       avail_cores_in_node);
