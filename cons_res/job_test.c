			      ...
			      bool test_only, bitstr_t *part_core_map)
{
	...
	List gres_list;
	info("D-_can_job_run_on_node- in");
	...
	info("D-_can_job_run_on_node- cpus:%d",cpus);
	return cpus;
}
...
bitstr_t *_make_core_bitmap(bitstr_t *node_map, uint16_t core_spec)
{
	...
	info("D-_make_core_bitmap- in ");
	nodes = bit_size(node_map);
	size = cr_get_coremap_offset(nodes);
	bitstr_t *core_map = bit_alloc(size);

		//bit_fmt(str,sizeof(str1),core_map);
		bit_fmt(str1,sizeof(str1),node_map);
		info("D-_make_core_bitmap- in node_map:%s,nodes:%d,size:%d, ",str1,nodes,size);
	for (n = 0; n < nodes; n++) {
		...
		bit_nset(core_map, c, coff-1);
			...
			bit_fmt(str,sizeof(str1),core_map);
			bit_fmt(str1,sizeof(str1),node_map);
		info("D-_make_core_bitmap- in core_map:%s,node_map:%s,nodes:%d,size:%d, ",str,str1,nodes,size);
		...
		if (!node_ptr->node_spec_bitmap) {
			/*info("D-_make_core_bitmap-CPUSpecList not registered for node %s yet",
			     node_ptr->name);*/
			continue;
		}
	...
	/*bit_fmt(str,sizeof(str1),core_map);
		...
		info("D-_make_core_bitmap- in core_map:%s ");*/
	return core_map;
}
			...
			bool prefer_alloc_nodes)
{
	...
	info("D");
	/*bit_fmt(str,sizeof(str),req_map);
	bit_fmt(str1,sizeof(str1),node_map);
	info("D-_eval_nodes- in node_map:%s,layout_ptr:%d,cr_node_cnt:%d"
	     ,str1,*layout_ptr,cr_node_cnt);*/
	xassert(node_map);
	...
	info("D-_eval_nodes- rem_cpus:%d,rem_nodes:%d,min_rem_node:%d",rem_cpus,rem_nodes,min_rem_nodes);
	...
	for (i = 0, ll = -1; i < cr_node_cnt; i++) {
		...
		if (bit_test(node_map, i)) {
			...
			if ((max_nodes > 0) && required_node) {
				...
				max_nodes--;
				info("D-_eval_nodes- rem_cpus:%d,rem_nodes:%d,min_rem_node:%d,max_nodes:%d"
				     ,rem_cpus,rem_nodes,min_rem_nodes,max_nodes);
				...
				bit_fmt(str1,sizeof(str1),node_map);
				info("D-_eval_nodes- in node_map:%s,avail_cpus:%d"
	 				,str1,avail_cpus);
			}
			 ...
			 bool prefer_alloc_nodes)
{
	...
	info("D-_choose_nodes- in ");
	if (job_ptr->details->req_node_bitmap){
		bit_fmt(str,sizeodf(str),job_ptr->details->req_node_bitmap);
		info("D-_choose_nodes- job_ptr->details->req_node_bitmap:%s",str);
	...
	for (i = 0; i < cr_node_cnt; i++) {
		...
		/* OR node has no CPUs */
			...
			info("D-_choose_nodes- reqmap:%s,job_ptr->details->whole_node:%d,job_ptr->details->max_cpus,cpu_cnt[i]"
				     ,str,job_ptr->details->whole_node,job_ptr->details->max_cpus,cpu_cnt[i]);
			
			if (reqmap && bit_test(reqmap, i)) {
				/* can't clear a required node! */
				bit_fmt(str,sizeodf(str),reqmap);
				info("D-_choose_nodes- reqmap:%s,job_ptr->details->whole_node:%d,job_ptr->details->max_cpus,cpu_cnt[i]"
				     ,str,job_ptr->details->whole_node,job_ptr->details->max_cpus,cpu_cnt[i]);
				return SLURM_ERROR;
			}
	...
	info("D-_choose_nodes- in max_nodes:%d,job_ptr->details->num_tasks:%d ",max_nodes,job_ptr->details->num_tasks);
	origmap = bit_copy(node_map);

	bit_fmt(str,sizeof(str),origmap);
	bit_fmt(str1,sizeof(str1),node_map);
	info("D-_choose_nodes- in origmap:%s,node_map:%s",str,str1);
	...
	for (i = 0; i < cr_node_cnt; i++) {
		//info("D-_choose_nodes- in cpu_cnt[i]:%d,most_cpus:%d ",cpu_cnt[i],most_cpus);
	...
	for (count = 1; count < most_cpus; count++) {
		...
		/*bit_fmt(str,sizeof(str),origmap);
		bit_fmt(str1,sizeof(str1),node_map);
		info("D-_choose_nodes- in origmap:%s,node_map:%s,cr_node_cnt:%d ",str,str1,cr_node_cnt);*/
		for (i = 0; i < cr_node_cnt; i++) {
	...
	FREE_NULL_BITMAP(origmap);
	//info("D-_choose_nodes- in ec:%d ",ec);
	return ec;
}
				...
				bool prefer_alloc_nodes)
{
	...
	char str[64],str1[64],str2[64];
	info("D-_select_nodes- in node_map");
	bit_fmt(str,sizeof(str),node_map);
	bit_fmt(str1,sizeof(str1),core_map);
	/*if(_assert_bitstr_valid(part_core_map)) {
		bit_fmt(str2,sizeof(str2),part_core_map);
	}
	else
		strcopy(str2,"NotDefined");*/
	info("D-_select_nodes- in node_map:%s,core_map:%s,",str,str1);
	if (bit_set_count(node_map) < min_nodes)
		return NULL;
	...
	bit_fmt(str1,sizeof(str1),core_map);
	//bit_fmt(str2,sizeof(str2),part_core_map);
	info("D-_select_nodes- in node_map:%s,core_map:%s,node_count:%d"
	     ,str,str1,cr_node_cnt);
	...
	if (details_ptr->ntasks_per_node && details_ptr->num_tasks) {
		...
		info("D-_select_nodes- in min_nodes:%d,i:%d", min_nodes,i);
	}
	...
	bit_fmt(str1,sizeof(str1),core_map);
	//bit_fmt(str2,sizeof(str2),part_core_map);
	info("D-_select_nodes-CN before in node_map:%s,core_map:%s",str,str1);
	...
	bit_fmt(str1,sizeof(str1),core_map);
	//bit_fmt(str2,sizeof(str2),part_core_map);
	info("D-_select_nodes-CN after in node_map:%s,core_map:%s",str,str1);
	...
	info("D-_select_nodes-  cpus:%d",cpus);
	return cpus;
}
			       ...
			       bitstr_t *new_core_bitmap)
{
	...
	info("D-_block_whole_nodes- in ");
	bit_fmt(str,sizeof(str),node_bitmap);
	bit_fmt(str1,sizeof(str1),orig_core_bitmap);
	bit_fmt(str2,sizeof(str2),new_core_bitmap);
	info("D-_block_whole_nodes- in node_bitmap:%s,orig_core_bitmap:%s,new_core_bitmap:%s",str,str1,str2);
			...
			bool qos_preemptor)
{
	...
	bitstr_t *origmap, *avail_cores, *free_cores, *part_core_map = NULL;
	...
	info("D-cr_job_test - Job_id:%d , select_node_cnt:%d ",job_ptr->job_id, cr_node_cnt);
	//_dump_state(cr_part_ptr);
	...
	bit_fmt(str,sizeof(str),node_bitmap);
	info("D-cr_job_test-after-_verify_node_state node_bitmap:%s",str);
	...
	if (details_ptr->min_cpus == details_ptr->min_nodes) {
		...
		info("D-cr_job_test- This is the case if -O/--overcommit  is true ");
		info("D-cr_job_test- details_ptr->min_cpus:%d",details_ptr->min_cpus);
		if ((mc_ptr->threads_per_core != (uint16_t) NO_VAL) &&
		    (mc_ptr->threads_per_core > 1)) {
			info("D-cr_job_test- mc_ptr->threads_per_core :%d",mc_ptr->threads_per_core );
			details_ptr->min_cpus *= mc_ptr->threads_per_core;
		}
		if ((mc_ptr->cores_per_socket != (uint16_t) NO_VAL) &&
		    (mc_ptr->cores_per_socket > 1)) {
			info("D-cr_job_test- mc_ptr->cores_per_socket :%d",mc_ptr->cores_per_socket );
	...
	if (select_debug_flags & DEBUG_FLAG_SELECT_TYPE) {
		info("D-cr_job_test -evaluating job %u on %u nodes and select_debug_flags:%d ,DEBUG_FLAG_SELECT_TYPE:%d",
		     job_ptr->job_id, bit_set_count(node_bitmap),select_debug_flags,DEBUG_FLAG_SELECT_TYPE);
	}

	origmap = bit_copy(node_bitmap);
	avail_cores = _make_core_bitmap(node_bitmap,
					job_ptr->details->core_spec);

	info("D-cr_job_test - after _make_core_bitmap");
	...
	bit_fmt(str,sizeof(str),origmap);
	bit_fmt(str1,sizeof(str1),avail_cores);
	bit_fmt(str2,sizeof(str2),free_cores);
	info("D-cr_job_test - node_bitmap(bitmap):%s,avail_cores:%s , free_cores: %s cpu_count: %d "
	 	   ,str,str1,str2,*cpu_count);
	...
	if (cpu_count == NULL) {
		/* job cannot fit */
		info("D-cr_job_test: cpu_count == NULL");
		FREE_NULL_BITMAP(origmap);
		FREE_NULL_BITMAP(free_cores);
		FREE_NULL_BITMAP(avail_cores);
		if (select_debug_flags & DEBUG_FLAG_SELECT_TYPE) {
			info("D-cr_job_test: test 0 fail: "
		...
		FREE_NULL_BITMAP(avail_cores);
		xfree(cpu_count);
		info("D-cr_job_test: test_only:%d,select_debug_flags:%d,DEBUG_FLAG_SELECT_TYPE:%d"
		     ,test_only,select_debug_flags,DEBUG_FLAG_SELECT_TYPE);
		if (select_debug_flags & DEBUG_FLAG_SELECT_TYPE)
			info("cons_res: cr_job_test: test 0 pass: test_only");
		return SLURM_SUCCESS;
	} else if (!job_ptr->best_switch) {
		info("D-cr_job_test:waiting for best switches");
		info("D-cr_job_test: test_only:%d,select_debug_flags:%d,DEBUG_FLAG_SELECT_TYPE:%d"
		     ,test_only,select_debug_flags,DEBUG_FLAG_SELECT_TYPE);
		FREE_NULL_BITMAP(origmap);
		FREE_NULL_BITMAP(free_cores);
	...
	if (cr_type == CR_MEMORY) {
		...
		info("D-cr_job_test: test_only:%d,select_debug_flags:%d,DEBUG_FLAG_SELECT_TYPE:%d"
		     ,test_only,select_debug_flags,DEBUG_FLAG_SELECT_TYPE);
		info("D-cr_job_test- cr_type == CR_MEMORY ");
		goto alloc_job;
	}
	xfree(cpu_count);
	if (select_debug_flags & DEBUG_FLAG_SELECT_TYPE) {
	...
	/*** Step 1 ***/
	bit_copybits(node_bitmap, origmap);
	bit_copybits(free_cores, avail_cores);

	bit_fmt(str,sizeof(str),origmap);
	info("D-_cr_job_test-step1 origmap:%s",str);
	bit_fmt(str,sizeof(str),node_bitmap);
	info("D-_cr_job_test-step1 node_bitmap:%s", str);
	bit_fmt(str,sizeof(str),avail_cores);
	info("D-_cr_job_test-step1 avail_cores:%s",str);
	bit_fmt(str,sizeof(str),free_cores);
	info("D-_cr_job_test-step1 free_cores:%s",str);
	
	if (exc_core_bitmap) {
		int exc_core_size  = bit_size(exc_core_bitmap);
		int free_core_size = bit_size(free_cores);
		info("D-_cr_job_test- exc_core_bitmap is available exc_core_size:%d,free_core_size:%d"
		...
		bit_fmt(str1,sizeof(str1),exc_core_bitmap);
		bit_fmt(str2,sizeof(str2),free_cores);
		info("D-cr_job_test - We are in step1 - exc_core_bitmap:%s , free_cores:%s",str1,str2);
	}
	if (exc_core_bitmap) {
#if _DEBUG
		char str[100];
		bit_fmt(str, (sizeof(str) - 1), exc_core_bitmap);
		debug2("D-cr_job_test - excluding cores reserved: %s", str);
#endif
		bit_not(exc_core_bitmap);
		bit_and(free_cores, exc_core_bitmap);
		bit_not(exc_core_bitmap);
		bit_fmt(str1,sizeof(str1),free_cores);
		info("D-cr_job_test-step1 (free_cores &=exc_core_bitmap):%s",str1);
	}
	...
	bit_fmt(str,sizeof(str),free_cores);
	info("D-_cr_job_test- free_cores:%s",str);
	for (p_ptr = cr_part_ptr; p_ptr; p_ptr = p_ptr->next) {
		...
		for (i = 0; i < p_ptr->num_rows; i++) {
			...
			if (p_ptr->part_ptr != job_ptr->part_ptr) {
				info("D-cr_job_test-step1 in p_ptr->part_ptr != job_ptr->part_ptr");
				//_dump_state(p_ptr);
				//_dump_state (job_ptr);
				continue;
			}
			if (part_core_map) {
				bit_fmt(str1,sizeof(str1),part_core_map);
				bit_fmt(str2,sizeof(str2),p_ptr->row[i].row_bitmap);
				info("D-cr_job_test-step1 in part_core_map part_core_map:%s p_ptr->row[i].row_bitmap:",str1,str2);
		...
		bit_fmt(str,sizeof(str),free_cores);
		info("D-_cr_job_test-after free_cores:%s",str);
		bit_fmt(str1,sizeof(str1),part_core_map);
	//	bit_fmt(str2,sizeof(str2),p_ptr->row[i].row_bitmap);
		info("D-cr_job_test-step1 part_core_map:%s",str1);
	}
	if (job_ptr->details->whole_node)
	...
	info("D-_cr_job_test-After step1 cpu_count:%d",cpu_count);

	if ((cpu_count) && (job_ptr->best_switch)) {
		/* job fits! We're done. */
		info("D-cr_job_test: test Step1 pass - "
	...
	if (job_node_req == NODE_CR_ONE_ROW) {
		 ...
		 * before this point. */
		if (select_debug_flags & DEBUG_FLAG_SELECT_TYPE) {
			info("D-cr_job_test: test 1 fail - "
	...
	bit_fmt(str,sizeof(str),origmap);
	bit_fmt(str1,sizeof(str1),avail_cores);
	info("D-cr_job_test:step2 origmap:%s ,avail_cores:%s",str,str1);
	...
	if (exc_core_bitmap) {
		...
		bit_not(exc_core_bitmap);
		bit_fmt(str1,sizeof(str1),free_cores);
		info("D-cr_job_test:step2 ~exc_core_bitmap:%s,free_cores &= exc_core_bitmap :%s,",str,str1);
	}
	...
	if (!jp_ptr) {
		fatal("D-cr_job_test :step2 could not find partition for job %u",
			job_ptr->job_id);
	 ...
	 * from avail_cores */
	if (select_debug_flags & DEBUG_FLAG_SELECT_TYPE) {
		info("D-cr_job_test:step2 looking for higher-priority or "
	...
	for (p_ptr = cr_part_ptr; p_ptr; p_ptr = p_ptr->next) {
		if ((p_ptr->part_ptr->priority <= jp_ptr->part_ptr->priority) &&
			...
			if (select_debug_flags & DEBUG_FLAG_SELECT_TYPE) {
				info("D-cr_job_test: continuing on "
				     "partition: %s partition_priority:%d", p_ptr->part_ptr->name,p_ptr->part_ptr->priority);
			}
			continue;
		}
		if (!p_ptr->row)
		...
		for (i = 0; i < p_ptr->num_rows; i++) {
			if (!p_ptr->row[i].row_bitmap)
				continue;
			bit_copybits(tmpcore, p_ptr->row[i].row_bitmap);
			bit_fmt(str,sizeof(str),tmpcore);
			bit_not(tmpcore); /* set bits now "free" resources */
			bit_and(free_cores, tmpcore);
			bit_fmt(str1,sizeof(str1),free_cores);
			info("D-cr_job_test- free_cores:%s , tmpcore:%s",str1,str);
		}
	}
	if (job_ptr->details->whole_node)
		_block_whole_nodes(node_bitmap, avail_cores, free_cores);
		...
		bit_fmt(str1,sizeof(str1),free_cores);
		bit_fmt(str1,sizeof(str),avail_cores);
		bit_fmt(str1,sizeof(str2),node_bitmap);
		info("D-cr_job_test- free_cores:%s , avail_cores:%s ,node_bitmap:%s",str1,str,str2);
	...
	if (!cpu_count) {
		...
		if (select_debug_flags & DEBUG_FLAG_SELECT_TYPE) {
			info("D-cr_job_test: test 2 fail - "
	...
	xfree(cpu_count);
	if (select_debug_flags & DEBUG_FLAG_SELECT_TYPE) {
		info("D-cr_job_test: test 2 pass - "
