...
s_p_options_t entity_options[] = {
	/* base keys */
        {"Position", S_P_UINT32},  // Bitmap position or Array Index correlation, This is used for conversion from Layout to Bitmap
        {"Selected", S_P_BOOLEAN},
		{"Allocated",S_P_BOOLEAN}, // Selected,Allocated separate list is possible
        {"NumNodes", S_P_UINT32},  // When the node inserted and deleted we have to corelate with the Layout
        {"NumCores", S_P_UINT32},
        {"NumJobs", S_P_UINT32},
		{"NumParts",S_P_UINT32},   // In root level only
        {"FreeNodes", S_P_UINT32},
        {"FreeCores", S_P_STRING},
		{"FreeSockets",S_P_UINT32}, // In node level only
        {"FreeMemory", S_P_UINT32}, // In node level only
		{"PartitionPriority",S_P_UINT32},
		{"ThreadsPerCore", S_P_UINT32},  //In the core entity only
		{"NodeState",S_P_LONG},
//After Allocation 
...
const layouts_keyspec_t keyspec[] = {
	/* base keys */
        {"Position", L_T_UINT32},  // Bitmap position or Array Index correlation, This is used for conversion from Layout to Bitmap
        {"Selected", L_T_BOOLEAN},
		{"Allocated",L_T_BOOLEAN}, // Selected,Allocated separate list is possible
        {"NumNodes", L_T_UINT32},  // When the node inserted and deleted we have to corelate with the Layout
        {"NumCores", L_T_UINT32},
        {"NumJobs", L_T_UINT32},
		{"NumParts",L_T_UINT32},   // In root level only
        {"FreeNodes", L_T_UINT32},
        {"FreeCores", L_T_STRING},
		{"FreeSockets",L_T_UINT32}, // In node level only
        {"FreeMemory", L_T_UINT32}, // In node level only
		{"PartitionPriority",L_T_UINT32},
		{"ThreadsPerCore", L_T_UINT32},  //In the core entity only
		{"NodeState",L_T_LONG},
//After Allocation 
		{"AllocatedMemory", L_T_UINT32},
