...
sub getrunning {
   my ($nrunpe, @fields);
    ...
    #my $line;
#    my $disco = `squeue -o %C`;
    open( LLS , "squeue-25 -o \'%C %N %t\' |grep R |sed \'s/x/ /g\' 2>/dev/null |");
#	open( LLS , "squeue-25 -o \'%C %N %t\' |grep R 2>/dev/null |");
   while ( <LLS> ) {
...
sub monitor_queues {
    ...
    $nque = 0;
    $nrun = 0;
    open( QSTAT, "squeue-25  2>/dev/null |");
    while (<QSTAT>) {
	@fields = split " ", $_;
        if  ($fields[4] eq "PD" ) {
  	  ++$nque;
...
sub submit {
    ...
    $subcmd = "sbatch-25 /home_nfs/georgioy/BENCHS/esp-2.2.1_8192/" . $_[1];
#    system("$subcmd");
#    $subcmd = $_[1];
    $doit   = $_[2];
    if (!defined($pid=fork())) {
...
sub create_jobs {
	...
	foreach my $j (keys %{$self->jobdesc}) {
		...
		for (my $i=0; $i < $jj[1]; $i++) {
			...
			my $NAS_CLASS='D';
			my $NPB='cg';
			if ($taskcount == 256){  $NAS_CLASS='D';} 
			if ($taskcount == 512){  $NAS_CLASS='D';}
			if ($taskcount == 784){ $NPB='ep'; $NAS_CLASS='D';}
			if ($taskcount == 1024){ $NAS_CLASS='E';}
			if ($taskcount == 1296){ $NPB='bt'; $NAS_CLASS='E';}
			if ($taskcount == 2048){ $NAS_CLASS='E';}
			if ($taskcount == 4096){ $NAS_CLASS='E';}
			if ($taskcount == 8192){ $NAS_CLASS='E';}
...
#SBATCH -n $nodes
#SBATCH -t $walltime
#SBATCH -o /home_nfs/georgioy/BENCHS/esp-2.2.1_8192/logs/$joblabel.out

#HOSTFILE="/tmp/nodes.\$SLURM_JOB_ID"
ESP=/home_nfs/georgioy/BENCHS/esp-2.2.1_8192/
#srun-25 -l /bin/hostname | sort -n | awk '{print \$2}' > /tmp/nodes.\$SLURM_JOB_ID

echo `\$ESP/bin/epoch` " START  $joblabel   Seq_\${SEQNUM}" >> \$ESP/LOG
$timer mpirun -np $np /home_nfs/georgioy/BENCHS/NPB3.3/NPB3.3-MPI/bin/$NPB.$NAS_CLASS.$np
