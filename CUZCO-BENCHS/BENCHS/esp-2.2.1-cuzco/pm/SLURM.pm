...
sub getrunning {
   my ($nrunpe, @fields);
    ...
    #my $line;
#    my $disco = `squeue -o %C`;
    open( LLS , "squeue -o \'%C %N %t\' |grep R |sed \'s/o/ /g\' 2>/dev/null |");
   while ( <LLS> ) {
...
#SBATCH -n $nodes
#SBATCH -t $walltime
#SBATCH -o /home_nfs/georgioy/BENCHS/esp-2.2.1-cuzco/logs/$joblabel.out
...
echo `\$ESP/bin/epoch` " START  $joblabel on \$SLURM_JOB_NODELIST  Seq_\${SEQNUM}" >> \$ESP/LOG
