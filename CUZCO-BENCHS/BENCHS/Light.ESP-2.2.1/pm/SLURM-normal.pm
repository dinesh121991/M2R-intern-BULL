...
sub getrunning {
   my ($nrunpe, @fields);
    ...
    #my $line;
#    my $disco = `squeue -o %C`;
    open( LLS , "squeue-24 -o \'%C %N %t\' |grep R |sed \'s/x/ /g\' 2>/dev/null |");
...
sub monitor_queues {
    ...
    $nque = 0;
    $nrun = 0;
    open( QSTAT, "squeue-24  2>/dev/null |");
    while (<QSTAT>) {
	@fields = split " ", $_;
        if  ($fields[4] eq "PD" ) {
  	  ++$nque;
...
sub submit {
    ...
    $subcmd = "sbatch-24 /home_nfs/georgioy/BENCHS/esp-2.2.1_512/" . $_[1];
#    system("$subcmd");
#    $subcmd = $_[1];
    $doit   = $_[2];
    if (!defined($pid=fork())) {
...
#SBATCH -n $nodes
#SBATCH -t $walltime
#SBATCH -o /home_nfs/georgioy/BENCHS/esp-2.2.1_512/logs/$joblabel.out

#HOSTFILE="/tmp/nodes.\$SLURM_JOB_ID"
ESP=/home_nfs/georgioy/BENCHS/esp-2.2.1_512/
