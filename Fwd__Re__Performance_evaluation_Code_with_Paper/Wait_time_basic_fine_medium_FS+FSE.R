...
#	png(file="Cumulated Distribution function on Wait time(OAR-SLURM).png",height=850,width=1000)
	...
	plot(ecdf(sort(Wait1)),pch=22, xlim=c(0,max(Wait1,Wait2,Wait3)),xlab="Wait time [s]",ylab="Jobs [%]", main= "CDF on waiting time for ESP benchmark upon 5040 nodes(16cpus/node) cluster  \n resource selection comparison")
...
#	mtext(paste("files :",F1, "-", F2)) 
	legend("topleft",c("cons_res_layout_no_overhead","cons_res_no_overhead","cons_res_layout_overhead"),pch=c(22,20,21),lty=c(1,0),inset=0.02)
	dev.off()
  ...
  #	colnames(m)<-h1$breaks[2:length(h1$breaks)]
	...
	mtext(paste("Files :",F1,"-",F2,"-",F3))	
  	...
  	legend("top",														# where to put the legend
  		...
  		c("cons_res_layout_no_overhead","cons_res_no_overhead","cons_res_layout_overhead"),	# what to write
...
Wait("cons_res_lay_no_overhead.swf","cons_res_no_overhead.swf","cons_res_lay_overhead.swf")
